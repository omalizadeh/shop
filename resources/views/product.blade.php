...
@section('load')
    <div class="page-title d-flex" aria-label="Page title" style="background-image: url({{ asset("view/img/page-title/shop-pattern.jpg") }}); max-height: 30px;">
        ...
        <div class="row">
            ...
            <div class="col-md-6 pb-5">
                <div class="product-gallery">
                    <span class="badge badge-{{ !empty($product->price) && $product->status != 'catalog' ? "success" : 'danger' }} text-md font-weight-normal">
                       {{ !empty($product->price) && $product->status != 'catalog' ? "قابل خرید" : 'امکان خرید انلاین وجود ندارد' }}
                    </span>
                    <div class="product-carousel owl-carousel">
                        @forelse($product->media as $item)
                            <a class="gallery-item" href="{{ asset($item['patch'].$item['name']) }}" data-fancybox="gallery{{ $loop->index }}" data-hash="four">
                                <img src="{{ asset($item['patch'].$item['name']) }}" alt="{{ $item['name'] }}"></a>
                        @empty
                        @endforelse
                    </div>
                    <ul class="product-thumbnails">
                        @forelse($product->media as $item)
                            <li><a href="#{{ $loop->index }}"><img src="{{ asset($item['patch'].$item['name']) }}" alt="{{ $item['name'] }}"></a></li>
                        @empty
                        @endforelse
                    </ul>
                </div>
            ...
            <div class="col-md-6 pb-5">
                ...
                <h3 class="h4 font-weight-light">
                  &nbsp;{{ !empty($product->price) && $product->status != 'catalog' ? number_format($product->price->amount) .' تومان ' : 'امکان خرید انلاین وجود ندارد' }}
                </h3>
                <p class="text-muted">{{ $product->limit_text }}...<a href='#details' class='scroll-to'>ادامه</a></p>
                    ...
                    <ul class="list-unstyled mb-4 pb-2">
                        @forelse($option['data'] as $property)
                            @if($loop->index >= 2)
                                @break
                            @endif
                            <li><strong>{{ $property['foo'] }} : </strong> {{ $property['bar'] }}</li>
                        @empty
                        @endforelse
                        <a href='#details' class='scroll-to'>ادامه</a>
                    </ul>
                @empty
                @endforelse
                <div class="row mt-4">
                    <div class="col-sm-6">
                        <span class="fild-value"> <img style="height: 55px;width: 100%;" alt="Barcoded value {{ $product->barcode }}" src="http://bwipjs-api.metafloor.com/?bcid=code128&text={{ $product->barcode }}"> </span>
                    </div>
                    <div class="col-sm-6">
                        @if(!empty($product->price) && $product->status != 'catalog')
                ...
                <div class="pt-1 mb-4"><span class="text-medium">شناسه محصول:</span> #{{ $product->barcode }}</div>
                <hr class="mb-2">
                <div class="d-flex flex-wrap justify-content-between align-items-center">
                    <div class="mt-2 mb-2">
                        <button class="btn btn-danger btn-sm my-2 mr-1" onclick="AddToFavorite()"><i class="fe-icon-heart"></i>&nbsp;افزودن به لیست علاقه مندی</button>
                        <button class="btn btn-info btn-sm my-2" data-toast data-toast-type="info" data-toast-position="topRight" data-toast-icon="fe-icon-info" data-toast-title="Product" data-toast-message="added to comparison table!"><i class="fe-icon-repeat"></i>&nbsp;مقایسه</button>
                    </div>
                    ...
                    <div class="alert alert-warning">
                        ...
                        <p class="text-justify">
                            ...
                            <b>
                                ...
                                <span style="direction: rtl;float: left;text-align: right;">&nbsp; تلفن تماس :&nbsp;</span>&nbsp;
                            </b>
                        </p>
                    </div>
                @endif
            ...
            <div class="row">
                ...
                <div class="col-md-6">
                    ...
                    <ul class="list-unstyled mb-4 pb-2">
                        ...
                        @empty
                        @endforelse
                            <li><strong>تولید کننده : </strong> <a href="{{ $product->brand->url }}" target="_blank">{{ $product->brand->name }}</a></li>
                            <li><strong>دسته بندی : </strong><a href="{{ $product->category->url }}" target="_blank">{{ $product->category->name }}</a></li>
                    </ul>
                    @empty
                    @endforelse
                </div>
        ...
        <div class="row pt-2">
            ...
            <div class="col-md-8 pb-5 mb-3">
                <div class="d-flex flex-wrap justify-content-between pb-2">
                    <h3 class="h4">بازخورد های اخیر</h3><a class="btn btn-primary btn-sm" href="{{ route("ProductComments" , ["slug" => $product->slug]) }}">مشاهده همه بازخورد ها</a>
                </div>
                <!-- Review-->
                <hr/>
