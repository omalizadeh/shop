                ...
                <div class="card-body p-0 d-flex">
                    <div class="d-flex flex-column m-auto">
                        <div class="stats-small__data text-center">
                            ...
                            <h6 class="stats-small__value count my-3">{{ $product->reviews->count() > 1 ? floatval($product->reviews->sum('range') / $product->reviews->count()) : '0' }}
                ...
                <ul class="list-group list-group-flush">
                    <li class="list-group-item pt-2 pb-2">
                        ...
                        <span class="d-flex">
                          ...
                          <strong class="text-danger font-weight-bold"> {{ is_null($product->storeroom) ? 'برای این محصول موجودی ثبت نشده است' : $product->storeroom->count .' عدد ' }}</strong>
                        </span>
                    </li>
                </ul>
                <button type="button" data-toggle="modal" data-target="#Storeroom" class="btn btn-success col-lg-12 rounded-0 rounded-bottom">
                        ...
                        <div class="modal-body">
                            <div class="form-row">
                                <div class="col-lg-6">
                                    ...
                                    <input required placeholder="1000000" type="text" value="" class="form-control" id="amountPrice" name="amount">
                                </div>
                                <div class="col-lg-6">
                                    <label for="barcode">
                                        شناسه محصول
                                    ...
                                    <input type="text" disabled="" value="{{ $product->barcode }}" class="form-control" id="barcode" name="barcode">
                                </div>
                            </div>
                        </div>
            ...
            <div id="Storeroom" class="modal fade" role="dialog">
                <div class="modal-dialog modal-lg">
                    ...
                    <div class="modal-content">
                            ...
                            <form>
                                <div class="form-row">
                                    <div class="col-lg-6">
                                        ...
                                        <input required placeholder="10" type="number" value="{{ empty($product->Storeroom) ? 0 : $product->Storeroom->count }}" class="form-control" id="count" name="count">
                                    </div>
                                    <div class="col-lg-6">
                                        <label for="barcode">
                                            شناسه محصول
                        ...
                        <div class="modal-footer">
                            <button type="button" class="btn btn-default" data-dismiss="modal">بستن</button>
                            <button type="button" class="btn btn-outline-success" onclick="StoreRoom()">بروزرسانی موجودی</button>
                        </div>
                    </div>
            ...
            <table class="table mb-0">
                ...
                @forelse($product->prices as $price)
                    <tr>
                        ...
                        <td>{{ $product->title }}</td>
                        <td>{{ $price->amount }}</td>
                        <td><span class="badge badge-{{ $price->StatusText['class'] }}">{{ $price->StatusText['text'] }}</span></td>
                        <td>{{ jdate($price->created_at)->format('l d S F Y') }}</td>
                        <td>
                            @if($price->status != 'new')
                                <button type="button" title="ثبت پیشفرض" onclick="defaultPrice('{{ $product->id }}','{{ $price->amount }}')" class="btn btn-outline-primary">
                                    <i class="material-icons">exit_to_app</i>
                                </button>
                                <button type="button" title="حدف قیمت" onclick="deletePrice('{{ $price->id }}')" class="btn btn-outline-danger">
                    ...
                    <div class="blog-comments__item d-flex p-3" id="Comment-{{ $comment }}">
                        ...
                        <div class="blog-comments__content">
                            <div class="blog-comments__meta text-muted">
                                ...
                                <span class="text-{{$comment->StatusText['class']}} font-weight-bold" id="Comment-{{$comment->id}}-Status">{{$comment->StatusText['text']}}</span>
                            </div>
                            ...
                            <div class="blog-comments__actions">
                              ...
                              <span class="text-success">
                                <i class="material-icons">check</i>
                                    ...
                                    <button type="button" class="btn btn-white" onclick="Comment('{{ $comment->id }}','delete')">
                              <span class="text-danger">
                                <i class="material-icons">clear</i>
                              </span> رد / حذف </button>
                                    <button type="button" class="btn btn-white" onclick="Comment('{{ $comment->id }}','replay')">
                    ...
                    <div class="blog-comments__item d-flex p-3">
                        ...
                        <div class="blog-comments__content">
                            <div class="blog-comments__meta text-muted">
                                ...
                                <span class="badge badge-{{ $review->range_text['bg'] }}">{{ $review->range_text['text'] }}</span>
                            </div>
                            <p class="m-0 my-1 mb-2 text-muted">
                                {{ $review->comment }}
                            </p>
